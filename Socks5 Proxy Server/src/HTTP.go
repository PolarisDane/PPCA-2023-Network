...
func ModifyForward(tarConn, srcConn net.Conn) {
	...
	for {
		...
		if tag > 0 {
			...
			if index != -1 {
				if contentLength == -1 {
					...
					if strings.HasSuffix(string(data[lens-5:lens]), "0\r\n\r\n") {
						//chunked传输的结尾为0\r\n\r\n
						data, lens = ResponseDecode(data, lens)
						if lens == -1 {
							return
						}
						tag = -1
			...
			if tag == -1 {
				data, lens = ResponseModify(data, lens)
...
func ResponseDecode(buf []byte, count int) ([]byte, int) {
	str := string(buf)
...
func ResponseModify(buf []byte, n int) ([]byte, int) {
	...
	data = bytes.Replace(data, []byte("百度"), []byte("Polaris_Dane"), -1)
